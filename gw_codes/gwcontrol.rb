...
#	res.execute_cmd(cmd, "Preparing the experiment", "Failed to prepare", "Progressing (4/5)")
  ...
  work :ccn_get_file do |res|
    cmd = "sshpass -p test #{SSH} -f -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"export PATH=$PATH:/usr/java/jdk1.7.0_07/bin:/usr/local/apache-ant-1.9.4/bin;source /etc/profile;ccngetfile ccnx:/snu.ac.kr/test ./testfile\""

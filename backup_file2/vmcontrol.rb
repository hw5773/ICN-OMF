...
#    FileUtils.chmod 1664, './tmp', :verbose => true
    Dir.mkdir('result') unless File.exists? 'result'
#    FileUtils.chmod 1664, './result', :verbose => true
...
#    sleep(time)
  ...
  work :ping_gw do |res|
    f = File.open("./result/#{res.property.id}_result.log", "a")
    f.write("ping to the gateway 3 times from #{res.property.sn} (#{res.property.vm_name})\n")

    `sshpass -p test scp -r ./result/#{res.property.id}_result.log root@#{res.property.manageIP}:~/`
    pwd = `pwd`

    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"echo \'before ping\' >> #{res.property.id}_result.log;date -u >> #{res.property.id}_result.log;ping -c 3 172.16.11.1;echo \'after ping\' >> #{res.property.id}_result.log;date -u >> #{res.property.id}_result.log;sshpass -p #{res.property.password} scp -r #{res.property.id}_result.log #{res.property.id}@#{res.property.ip}:#{pwd}/#{res.property.id}_result.log\""
  ...
  work :ccn_get_file do |res|
    f = File.open("./result/#{res.property.id}_result.log", "a")
    f.write("request #{res.property.target_file} from #{res.property.sn} (#{res.property.uid})\n")

    `sshpass -p test scp -r result/#{res.property.id}_result.log root@#{res.property.manageIP}:~/`
    pwd = `pwd`
    
    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"export PATH=$PATH:/usr/java/jdk1.7.0_07/bin:/usr/local/apache-ant-1.9.4/bin;source /etc/profile;echo \'before ccn get\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;ccngetfile #{res.property.target_file} #{res.property.output_file};echo \'after ccn get\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;sshpass -p #{res.property.password} scp -r #{res.property.id}_result.log #{res.property.id}@#{res.property.ip}:#{pwd}/result/#{res.property.id}_result.log\""
  ...
  work :ccn_get_ip do |res|
    f = File.open("./result/#{res.property.id}_result.log", "a")
    f.write("request #{res.property.target_file} and send back to IP network from #{res.property.sn} (#{res.property.uid})\n")

    `sshpass -p test scp -r ./result/#{res.property.id}_result.log root@#{res.property.manageIP}:~/`
    pwd = `pwd`
    
    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"export PATH=$PATH:/usr/java/jdk1.7.0_07/bin:/usr/local/apache-ant-1.9.4/bin;source /etc/profile;echo \'before ccn get\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;ccngetfile #{res.property.target_file} #{res.property.output_file};echo \'after ccn get\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;\""
    res.execute_cmd(cmd, "Getting the file #{res.property.target_file}", "Failed", "ccnget success! now it will be sent back")
    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"echo \'before send the output file to IP network\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log; sshpass -p #{res.property.back_password} scp -r #{res.property.output_file} #{res.property.back_id}@#{res.property.back}:~/#{res.property.output_file}; echo \'before send the output file to IP network\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;sshpass -p #{res.property.password} scp -r #{res.property.id}_result.log #{res.property.id}@#{res.property.ip}:~/#{pwd}/result\""
  ...
  work :ccn_put_file do |res|
    f = File.open("./result/#{res.property.id}_result.log", "a")
    f.write("put #{res.property.put_file} into #{res.property.repoName} from #{res.property.sn} (#{res.property.uid})\n")

    `sshpass -p test scp ./result/#{res.property.id}_result.log root@#{res.property.manageIP}:~/`
    pwd = `pwd`
    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"export PATH=$PATH:/usr/java/jdk1.7.0_07/bin:/usr/local/apache-ant-1.9.4/bin;source /etc/profile;echo \'before ccn put\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;ccnputfile #{res.property.repoName}/#{res.property.put_file} #{res.property.put_file}; echo \'after ccn put\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;sshpass -p #{res.property.password} scp -r #{res.property.id}_result.log #{res.property.id}@#{res.property.ip}:~/#{pwd}/result\""
  ...
  work :ping_to do |res|
    ...
    f = File.open("./result/#{res.property.id}_result.log", "a")
    f.write("ping from #{res.property.sn} (#{res.property.uid}) to #{res.property.target}\n")
    ...
    cmd = "sshpass -p test #{SSH} -o StrictHostKeyChecking=no root@#{res.property.manageIP} \"echo \'before ping\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;ping -c 3 #{list[1]}; echo \'after ccn put\' >> #{res.property.id}_result.log; date -u >> #{res.property.id}_result.log;sshpass -p #{res.property.password} scp -r #{res.property.id}_result.log #{res.property.id}@#{res.property.ip}:~/#{res.property.output_file}\""
